/// from https://stackoverflow.com/questions/58025730/how-to-determine-if-a-net-assembly-was-built-with-platform-target-anycpu-anycp/58025731#58025731
    ...
    public class CorFlagsReader
    {
        ...
        public static CorFlagsReader ReadAssemblyMetadata(Stream stream)
        {
            ...
            using (var reader = new BinaryReader(stream, new UTF8Encoding(), true))
            {
                ...
                var peFormat = (PEFormat)reader.ReadUInt16();
                ...
                for (int i = 0; i < numberOfSections; i++)
                {
                    ...
                    var section = new Section { VirtualSize = reader.ReadUInt32(), VirtualAddress = reader.ReadUInt32() };
                    reader.ReadUInt32();
                ...
                var corflags = (CorFlags)reader.ReadUInt32();
